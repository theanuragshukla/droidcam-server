...
app.get("/fetchImage", async (req, res) => {
  try {
    ...
    res.json({ status: false, msg: "some error occoured" });
  }
});
...
app.post("/upload-image", async (req, res) => {
  try {
    ...
    await new imageSchema(final_img).save();
    const doc = await storageSchema.findOne({ uid: req.usrProf.uid });
    if (doc) {
      ...
      doc
        ...
        .then((result) => {
          res
            .status(200)
            .json({ status: true, msg: "image uploaded successfullt" });
        })
        .catch((err) => {
          res.json({
            status: false,
